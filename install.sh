...
# Usage: curl -fsSL https://raw.githubusercontent.com/hivoid-org/hivoid-hub/main/install.sh | sudo bash
# =========================================================
set -e
export LANG=C.UTF-8
...
# ── Prompt helpers ────────────────────────────────────────
prompt()  { printf "  ${CYAN}?${NC}  $1 "; read -r "$2"; }
promptS() { printf "  ${CYAN}?${NC}  $1 "; read -rs "$2"; echo; }
...
# ── Zip or Auto-Download ──────────────────────────────────
ZIP_FILE="${1:-hivoid-hub-release.zip}"

if [[ ! -f "$ZIP_FILE" ]]; then
  info "Local release archive not found. Fetching latest from GitHub..."
  # Require curl for API fetch
  if ! command -v curl &> /dev/null; then
    apt-get update -qq && apt-get install -y -qq curl > /dev/null 2>&1
  fi
  
  API_URL="https://api.github.com/repos/hivoid-org/hivoid-hub/releases/latest"
  DOWNLOAD_URL=$(curl -s $API_URL | grep '"zipball_url":' | sed -E 's/.*"([^"]+)".*/\1/')
  ...
  spin_start "Downloading latest release..."
  curl -fsSL -o "$ZIP_FILE" "$DOWNLOAD_URL"
  spin_stop; ok "Downloaded latest release"
fi

info "Release archive : ${BOLD}$ZIP_FILE${NC}"
...
# Handle GitHub zipballs which have a root directory wrapper
if [[ $(ls -1 "$TMP_EXTRACT" | wc -l) -eq 1 ]]; then
  EXTRACTED_DIR=$(ls -1 "$TMP_EXTRACT")
  mv "$TMP_EXTRACT/$EXTRACTED_DIR"/* "$INSTALL_DIR"/
  mv "$TMP_EXTRACT/$EXTRACTED_DIR"/.[!.]* "$INSTALL_DIR"/ 2>/dev/null || true
else
  mv "$TMP_EXTRACT"/* "$INSTALL_DIR"/
  mv "$TMP_EXTRACT"/.[!.]* "$INSTALL_DIR"/ 2>/dev/null || true
fi
rm -rf "$TMP_EXTRACT"
...
# ── Step 8 — Management Terminal ──────────────────────────
echo ""
info "Configuring Management Terminal..."
MANAGER_PATH="$INSTALL_DIR/backend/scripts/manager.py"
chmod +x "$MANAGER_PATH"

# Create a wrapper script in /usr/local/bin for global access
cat > /usr/local/bin/hivoid-hub <<EOF
#!/usr/bin/env bash
# HiVoid Hub Management Wrapper
$INSTALL_DIR/backend/venv/bin/python3 $MANAGER_PATH "\$@"
EOF
chmod +x /usr/local/bin/hivoid-hub

# Create aliases
ln -sf /usr/local/bin/hivoid-hub /usr/local/bin/HiVoid-hub
ln -sf /usr/local/bin/hivoid-hub /usr/local/bin/hihub

ok "Management Terminal installed as ${BOLD}hivoid-hub${NC}, ${BOLD}HiVoid-hub${NC}, and ${BOLD}hihub${NC}"
echo ""
